...
/* Exercise 4-9. Our getch and ungetch do not handle a pushed-back EOF correctly. Decide what their properties ought to be if
an EOF is pushed back, then implement your design. */

/* The solution here is to change the buffer to an int array instead of char.
The reason is because EOF is seen as -1 from getchar(). 
char is not guranteed to be signed but int is by default signed.  */
